...
void GuildHouseCatalogMgr::Load()
{
    ...
    if (QueryResult result = WorldDatabase.Query("SELECT componentId, catalogId, spawnFlags, behaviorFlags, entry, displayId, scale, scriptType, scriptData, xOffset, yOffset, zOffset, oOffset, targetMap, targetX, targetY, targetZ, targetO, childCatalogId, sortOrder FROM guildhouse_catalog_asset ORDER BY sortOrder"))
    {
        do
        {
            Field* fields = result->Fetch();
            ...
            component.TargetX = fields[14].IsNull() ? 0.f : fields[14].Get<float>();
            component.TargetY = fields[15].IsNull() ? 0.f : fields[15].Get<float>();
            component.TargetZ = fields[16].IsNull() ? 0.f : fields[16].Get<float>();
            component.TargetO = fields[17].IsNull() ? 0.f : fields[17].Get<float>();
            component.ChildCatalogId = fields[18].IsNull() ? 0 : fields[18].Get<uint32_t>();
            component.SortOrder = fields[19].Get<uint16_t>();
            itr->second.Components.push_back(component);
        } while (result->NextRow());
    }

    LOG_INFO("server.loading", "GuildHouseCatalogMgr loaded {} categories and {} catalog items", _categories.size(), _catalogs.size());
}
