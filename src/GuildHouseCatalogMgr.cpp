...
void GuildHouseCatalogMgr::Load()
{
    ...
    if (QueryResult result = WorldDatabase.Query(
        "SELECT catalogId, categoryId, name, "
        ...
        "FROM guildhouse_catalog"))
    {
        do
        {
            ...
            catalog.SpawnFlags =
                static_cast<GHSpawnFlags>(
                    fields[3].Get<uint32_t>());


            catalog.BehaviorFlags =
                static_cast<GHBehaviorFlags>(
    ...
    if (QueryResult result = WorldDatabase.Query(
        "SELECT componentId, catalogId, "
        "spawnFlags, behaviorFlags, "
        ...
        "ORDER BY sortOrder"))
    {
        do
        {
            ...
            component.ScriptType =
                static_cast<GHScriptType>(
                    fields[7].Get<uint32_t>());
    ...
    LOG_INFO(
        "module",
        "GuildHouseCatalogMgr loaded {} categories and {} catalog items",
        _categories.size(),
        _catalogs.size());
}
...
const GHCatalog*
GuildHouseCatalogMgr::GetCatalog(uint32_t catalogId) const
{
    auto itr = _catalogs.find(catalogId);
...
const GHCategory*
GuildHouseCatalogMgr::GetCategory(uint32_t categoryId) const
{
...
GuildHouseCatalogMgr::GetRootCategories() const
{
