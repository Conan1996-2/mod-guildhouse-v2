...
private:
    ...
    bool SpawnCreature(uint32_t guildId, uint32_t assetId, uint32_t entry, float x, float y, float z, float o);

    bool SpawnGameObject( uint32_t guildId, uint32_t assetId, uint32_t entry, float x, float y, float z, float o);
